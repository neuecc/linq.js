...
{
    actual = Enumerable.make(1).CascadeDepthFirst("$+$").Take(7).ToArray();
    deepEqual(actual, [1, 2, 4, 8, 16, 32, 64]);
    actual = Enumerable.make(1).CascadeDepthFirst("$+$", "v,nl=>{v:v,nl:nl}").Take(3).ToArray();
    deepEqual(actual, [{ v: 1, nl: 0 }, { v: 2, nl: 1 }, { v: 4, nl: 2}]);
});

test("CascadeBreadthFirst", function ()
{
    actual = Enumerable.make(1).CascadeBreadthFirst("$+$").Take(7).ToArray();
    deepEqual(actual, [1, 2, 4, 8, 16, 32, 64]);
    actual = Enumerable.make(1).CascadeBreadthFirst("$+$", "v,nl=>{v:v,nl:nl}").Take(3).ToArray();
    deepEqual(actual, [{ v: 1, nl: 0 }, { v: 2, nl: 1 }, { v: 4, nl: 2}]);
});
